...
import com.bradrydzewski.gwt.calendar.client.dayview.TimeBlock;
...
public class DayViewLayoutStrategy {
    ...
    public ArrayList<AppointmentAdapter> doLayout(List<Appointment> appointments, int dayIndex, int dayCount) {
        ...
        int numberOfTimeBlocks = MINUTES_PER_HOUR / minutesPerInterval * (settings.getSettings().getWorkingHourEnd() - settings.getSettings().getWorkingHourStart());
        ...
        for (Appointment appointment : appointments) {
            ...
            apptCell.setAppointmentStart(minutesSinceDayStarted(appointment.getStart()));
            apptCell.setAppointmentEnd(minutesSinceDayStarted(appointment.getEnd()));
            ...
            // pas de startBlock si rdv en dehors des horaires de travail
            if (startBlock == null) {
                continue;
            }
        ...
        /*
         * for(AppointmentCell apptCell : appointmentCells) { for (TimeBlock block : apptCell.getIntersectingBlocks()) { int maxCol = 0; //find the max cell for
         * (AppointmentCell apptCell : block.getAppointments()) { int col = apptCell.getColumnStart(); if (col > maxCol) { maxCol = col; } }
    ...
    private int minutesSinceDayStarted(Date day) {
        return (day.getHours() - settings.getSettings().getWorkingHourStart()) * 60 + day.getMinutes();
    }
